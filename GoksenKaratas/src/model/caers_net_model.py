...
class FusionNetwork(nn.Module):
    def __init__(self, use_face=True, use_context=True, num_class=7, use_attention=True):
        ...
        if use_attention:
            ...
            self.face_stream_conv = CNN2DBlock(conv_num=2, cnn_num_kernels=[256, 128, 1], cnn_kernel_size=1, bn=False,
                                               relu=False, maxpool=False)
            self.context_stream_conv = CNN2DBlock(conv_num=2, cnn_num_kernels=[256, 128, 1], cnn_kernel_size=1,
        ...
        self.conv1 = nn.Conv2d(256 * 2, 128, kernel_size=1) if (use_face and use_context) else nn.Conv2d(256, 128,
                                                                                                         kernel_size=1)
